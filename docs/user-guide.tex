...
\begin{tipbox}
    ...
    Volume and RSSI changes flash briefly for visual confirmation.
\end{tipbox}
...
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{4cm} X@{}}
    ...
    \midrule
    FM stereo blend & Strong signals preserve stereo; weak blend to mono (was inverted). \\
    RSSI intensity & Weak signals now correctly get MORE noise/fading (math was backwards). \\
    Double degradation & Removed redundant second noise layer from the audio pipeline. \\
    Pink noise clicks & Persistent generator state --- no 23ms chunk-boundary artifacts. \\
    Thread race & Fixed crash-on-stop timing between refill and main threads. \\
    N key & Now actually skips to the next track. \\
    Arrow keys & Works across xterm, tmux, SSH (application + normal mode). \\
    Pre-flight check & Clean error if ffmpeg/ffplay missing instead of silent failure. \\
    Headless mode & Now displays now-playing track info during playback. \\
    ...
    Metadata sync & Lock-guarded MP3Source access --- no race conditions. \\
...
{\small\textcolor{radioGray}{RadioSim {\tiny —} github.com/EnigmaK9/radio-sim {\tiny —} July 2026}}\\[1mm]
